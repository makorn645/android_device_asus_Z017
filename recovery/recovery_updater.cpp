...
#define BL_PART_PATH "/dev/block/bootdevice/by-name/sbl1"
#define MODEM_PART_PATH "/dev/block/bootdevice/by-name/modem"
#define TZ_PART_PATH "/dev/block/bootdevice/by-name/tz"
#else
#define BL_PART_PATH "/dev/block/platform/soc/7824900.sdhci/by-name/sbl"
#define MODEM_PART_PATH "/dev/block/platform/soc/7824900.sdhci/by-name/modem"
#define TZ_PART_PATH "/dev/block/platform/soc/7824900.sdhci/by-name/tz"
#endif
#define BL_VER_STR "QC_IMAGE_VERSION_STRING=BOOT.BF."
#define BL_VER_STR_LEN 32
#define MODEM_VER_STR "QC_IMAGE_VERSION_STRING=MPSS.TA."
#define MODEM_VER_STR_LEN 32
#define TZ_VER_STR "QC_IMAGE_VERSION_STRING=TZ.BF."
#define TZ_VER_STR_LEN 30
#define IMAGE_VER_BUF_LEN 255
...
static int get_bl_version(char *ver_str, size_t len) {
    int ret = 0;
    int fd;
    int bl_size;
    char *bl_data = NULL;
    ...
    fd = open(BL_PART_PATH, O_RDONLY);
    if (fd < 0) {
        ret = errno;
        goto err_ret;
    }

    bl_size = lseek64(fd, 0, SEEK_END);
    if (bl_size == -1) {
        ret = errno;
        goto err_fd_close;
    }

    bl_data = (char *) mmap(NULL, bl_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (bl_data == (char *)-1) {
        ret = errno;
        goto err_fd_close;
    }

    /* Do Boyer-Moore search across BL data */
    offset = bm_search(bl_data, bl_size, BL_VER_STR, BL_VER_STR_LEN);
    if (offset != NULL) {
        snprintf(ver_str, len, "%s", offset + BL_VER_STR_LEN);
    } else {
        ret = -ENOENT;
    }

    munmap(bl_data, bl_size);
err_fd_close:
    close(fd);
err_ret:
    return ret;
...
static int get_tz_version(char *ver_str, size_t len) {
    ...
    if (offset != NULL) {
        snprintf(ver_str, len, "%s", offset + TZ_VER_STR_LEN);
    } else {
        ret = -ENOENT;
    }
...
/* verify_bootloader("BL_VERSION", "BL_VERSION", ...) */
Value * VerifyBootloaderFn(const char *name, State *state, const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_bl_version[IMAGE_VER_BUF_LEN];
    size_t i;
    int ret;

    ret = get_bl_version(current_bl_version, IMAGE_VER_BUF_LEN);
    if (ret) {
        return ErrorAbort(state, kFreadFailure,
                "%s() failed to read current Bootloader build version: %d", name, ret);
    }

    std::vector<std::string> bl_version;
    if (!ReadArgs(state, argv, &bl_version)) {
        return ErrorAbort(state, kArgsParsingFailure, "%s() error parsing arguments", name);
    }

    ret = 0;
    for (i = 0; i < argv.size(); i++) {
        uiPrintf(state, "Checking for Bootloader version %s\n", bl_version[i].c_str());

        if (strncmp(bl_version[i].c_str(), current_bl_version, strlen(bl_version[i].c_str())) == 0) {
            ret = 1;
            break;
		}
    }
...
Value * VerifyModemFn(const char *name, State *state, const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_modem_version[IMAGE_VER_BUF_LEN];
    size_t i;
    int ret;

    ret = get_modem_version(current_modem_version, IMAGE_VER_BUF_LEN);
    if (ret) {
        return ErrorAbort(state, kFreadFailure,
                "%s() failed to read current MODEM build version: %d", name, ret);
    }
    ...
    for (i = 0; i < argv.size(); i++) {
        ...
        if (strncmp(modem_version[i].c_str(), current_modem_version, strlen(modem_version[i].c_str())) == 0) {
            ret = 1;
            break;
        }
    }
...
Value * VerifyTrustZoneFn(const char *name, State *state, const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_tz_version[IMAGE_VER_BUF_LEN];
    size_t i;
    int ret;

    ret = get_tz_version(current_tz_version, IMAGE_VER_BUF_LEN);
    if (ret) {
        return ErrorAbort(state, kFreadFailure,
                "%s() failed to read current TrustZone build version: %d", name, ret);
    }
    ...
    ret = 0;
    for (i = 0; i < argv.size(); i++) {
        uiPrintf(state, "Checking for TrustZone version %s\n", tz_version[i].c_str());
...
void Register_librecovery_updater_msm8953() {
    RegisterFunction("msm8953.verify_bootloader", VerifyBootloaderFn);
    RegisterFunction("msm8953.verify_modem", VerifyModemFn);
    RegisterFunction("msm8953.verify_trustzone", VerifyTrustZoneFn);
}
